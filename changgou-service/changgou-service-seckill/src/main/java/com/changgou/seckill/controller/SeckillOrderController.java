...
public class SeckillOrderController {
    ...
    public Result add(String time, Long id){
        ...
        String username ="xiaoxiong";
        Boolean bo = seckillOrderService.add(time,id,username);
        if(bo){
            //抢单成功
            return new Result(true,StatusCode.OK,"已进入排队状态！");
        }else {
    ...
    public Result<SeckillStatus> queryStatus(String username){
        try {
            ...
            return new Result<>(true,StatusCode.OK,"查询当前登录用户的抢单信息OK",seckillStatus);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return new Result<>(true,StatusCode.ERROR,"查询当前登录用户的抢单信息失败");
    }
}
